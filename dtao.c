...
static struct wl_surface *popup_wl_surface;
static struct wl_surface *active_wl_surface;

static int32_t output = -1;

static uint32_t popup_x, popup_y, popup_width, popup_height;
static uint32_t popup_stride, popup_bufsize;
...
popup_draw_frame()
{
	...
	int fd = allocate_shm_file(popup_bufsize);
	if (fd == -1)
		return NULL;

	uint32_t *data = mmap(NULL, popup_bufsize,
	...
	struct wl_shm_pool *pool = wl_shm_create_pool(shm, fd, popup_bufsize);
	struct wl_buffer *buffer = wl_shm_pool_create_buffer(pool, 0,
			popup_width, popup_height, popup_stride, WL_SHM_FORMAT_ARGB8888);
	...
	pixman_image_t *bar = pixman_image_create_bits(PIXMAN_a8r8g8b8,
			popup_width, popup_height, data, popup_width * 4);

	pixman_image_fill_boxes(PIXMAN_OP_SRC, bar, &fgcolor, 1,
			&(pixman_box32_t) {.x1 = 0, .x2 = popup_width, .y1 = 0, .y2 = popup_height});

	pixman_image_unref(bar);
	munmap(data, popup_bufsize);
	return buffer;
}
...
static void xdg_surface_handle_configure(void *data,
	...
	fprintf(stderr, "XDG Surface configured\n");
	
	xdg_surface_ack_configure(xdg_surface, serial);
	
	fprintf(stderr, "START DRAW FRAME\n");
	struct wl_buffer *buffer = popup_draw_frame();
	fprintf(stderr, "END DRAW FRAME\n");

	if (!buffer)
		return;

	wl_surface_attach(popup_wl_surface, buffer, 0, 0);
	wl_surface_damage(popup_wl_surface, 0, 0, popup_width, popup_height);
	wl_surface_commit(popup_wl_surface);

	fprintf(stderr, "Committed to surface\n");
...
static void xdg_popup_configure(void *data, struct xdg_popup *xdg_popup,
	...
	fprintf(stderr, "XDG_POPUP_CONFIGURE START %d %d\n", w, h);
	popup_width = w;
	popup_height = h;
	popup_x = x;
	popup_y = y;
	popup_stride = popup_width * 4;
	popup_bufsize = popup_stride * popup_height;
	fprintf(stderr, "XDG_POPUP_CONFIGURE END x:%d y:%d mx:%d my:%d\n", popup_x, popup_y, mousex, mousey);
}
...
popup_destroy(void) {
	if(popup_wl_surface) {
		fprintf(stderr, "Actually destroying popup\n");
...
popup_create(uint32_t serial) {
	...
	if(!xdg_surface || !xdg_positioner)
		BARF("xdg_surface");

	xdg_positioner_set_size(xdg_positioner, 256, 256);
	xdg_positioner_set_offset(xdg_positioner, 0, 200);
	xdg_positioner_set_anchor_rect(xdg_positioner, mousex, mousey, 1, 1);
	xdg_positioner_set_anchor(xdg_positioner, XDG_POSITIONER_ANCHOR_TOP_RIGHT);
		...
		uint32_t state)
{
	...
	} else if(active_wl_surface == popup_wl_surface && !state) {
		popup_destroy();
	}
}
